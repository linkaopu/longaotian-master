...
DATA_DIR = os.path.join('images', 'save') + os.sep  # input images after preprocessing
DEBUG_DIR = os.path.join('images', 'debug') + os.sep  # visualization outputs
PATTERN_SIZE = (5, 7)  # (9, 6)
SQUARE_SIZE = 1.5  # mm   #1.0 #
...
def get_camera_images():
    # Ensure directories exist (DATA_DIR may be read-only; DEBUG_DIR is for outputs)
    if not os.path.isdir(os.path.dirname(DATA_DIR.rstrip(os.sep))):
        os.makedirs(os.path.dirname(DATA_DIR.rstrip(os.sep)), exist_ok=True)
    os.makedirs(DEBUG_DIR, exist_ok=True)
    images = [each for each in glob.glob(DATA_DIR + "*.png")]
    images = sorted(images)
    for each in images:
        grayImage = cv2.imread(each, 0)
        # ret, thresh =cv2.threshold(grayImage, 45, 255,cv2.THRESH_BINARY)
        # cv2.imwrite(each[:-4]+'1'+'.png', thresh)
        # yield (each, thresh)
        # medianblur = cv2.medianBlur(grayImage, 5)
        # cv2.imwrite(each[:-4]+'1'+'.png',medianblur)
        # yield (each,medianblur)
        yield (each, grayImage)
...
def getChessboardCorners(images=None, visualize=True):
    ...
    for (path, each) in get_camera_images():  # images:
        ...
        ret, corners = cv2.findChessboardCorners(each, patternSize=PATTERN_SIZE, flags=cv2.CALIB_CB_CLUSTERING)
        if ret:
...
def get_intrinsic_parameters(H_r):
    ...
    alpha = np.sqrt(np.abs(l / b[0]))
    beta = np.sqrt(np.abs((l * b[0]) / (b[0] * b[2] - b[1] ** 2)))
